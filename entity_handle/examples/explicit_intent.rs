...
/*
This example does the same as simple.rs, but uses `Intent` to control how an asset is processed.
*/
...
mod number_asset {
    ...
    pub fn plugin(app: &mut App) {
        ...
        app.register_intent::<PPrintRepresentation>();
        app.register_intent::<RawRepresentation>();
    }
    ...
    fn generate_ppr(
        ...
        assets: Query<
            (Entity, &NumberAsset),
            (
                With<IntentMarker<PPrintRepresentation>>, // NEW!
                Without<PPrintRepresentation>,
            ),
        >,
    ) {
    ...
    fn generate_raw(
        ...
        assets: Query<
            (Entity, &NumberAsset),
            (
                With<IntentMarker<RawRepresentation>>, // NEW!
                Without<RawRepresentation>,
            ),
        >,
    ) {
        assets.iter().for_each(|(e, na)| {
...
mod game {
    ...
    #[derive(Resource)]
    struct Assets {
        a42: EntityAssetHandle<NumberAsset, PPrintRepresentation>, // NEW!
        a69: EntityAssetHandle<NumberAsset, PPrintRepresentation>, // NEW!
    }
